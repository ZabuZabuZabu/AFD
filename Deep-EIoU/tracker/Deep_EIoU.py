...
class Deep_EIoU(object):
    ...
    def update(self, output_results, embedding):
        ...
        # # デバッグ用に検出結果を表示　クリア
        # logger.info(f"det shape {output_results.shape}")
        ...
        if len(output_results):
            if output_results.shape[1] == 5:
                ...
                scores = output_results[:, 4] * output_results[:, 5]#どうやらスコアが二個ある、その２つをかけたものがマッチングに使われるスコアになる
                bboxes = output_results[:, :4]
            else:
        ...
        for iteration in range(num_iteration):
            ...
            if self.args.with_reid:
                ...
                #count how many times EMB and IoU used
                self.iou_count = getattr(self, 'iou_count', [])
                self.emb_count = getattr(self, 'emb_count', [])

                iou_selected = (ious_dists < emb_dists).sum()  # IoUが選ばれた回数
                emb_selected = (emb_dists <= ious_dists).sum()  # Embeddingが選ばれた回数

                self.iou_count.append(iou_selected)
                self.emb_count.append(emb_selected)
...
def plot_matching_stats(iou_counts, emb_counts):
    plt.figure(figsize=(10, 5))
    plt.plot(iou_counts, label="IoU-based Matches", marker='o')
    plt.plot(emb_counts, label="Embedding-based Matches", marker='s')
    plt.xlabel("Frame")
    plt.ylabel("Count")
    plt.legend()
    plt.title("IoU vs Embedding Matching Counts Over Time")
    plt.grid()
    plt.show()
