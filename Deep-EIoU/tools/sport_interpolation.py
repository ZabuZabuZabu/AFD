...
if __name__ == '__main__':
    ...
    file_path = os.path.join(args.root_path,'SCT_DeepEIoU1')

    mkdir_if_missing(os.path.join(args.root_path,'interpolation_DeepEIoU1'))
        
    for scene in os.listdir(file_path):
        scene = scene.replace('.txt','')
        input_path = os.path.join(file_path,'{}.txt'.format(scene))
        output_path = os.path.join(args.root_path,'interpolation_DeepEIoU1','{}.txt'.format(scene))
