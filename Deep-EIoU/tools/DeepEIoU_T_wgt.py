...
def make_parser():
    ...
    parser.add_argument(
        "--save_num_emb_iou",
        default=False,
        help="whether to save the inference result of count num of How many times emb and iou used",
    )
...
def plot_matching_stats(iou_count, emb_count, output_dir="results", filename="matching_stats.png"):
    """
    IoUとEmbeddingの使用回数を時系列的に可視化し、指定ディレクトリに画像として保存する。
    """
    os.makedirs(output_dir, exist_ok=True)
    save_path = os.path.join(output_dir, filename)

    plt.figure(figsize=(12, 6))  # グラフの横幅を広げる

    # IoU Matching
    plt.plot(iou_count, label="IoU Matching", color="blue", marker="o", markersize=4, linewidth=1.5, alpha=0.8)

    # Embedding Matching
    plt.plot(emb_count, label="Embedding Matching", color="red", marker="o", markersize=4, linewidth=1.5, alpha=0.5)  # 透明度を下げる

    plt.xlabel("Frame")
    plt.ylabel("Count")
    plt.title("Matching Method Usage Over Time")
    plt.legend()
    plt.grid(True)
    
    # X軸の範囲をフレーム数に合わせる
    plt.xlim(0, len(iou_count))

    # 画像として保存
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()

    print(f"マッチング統計のグラフを '{save_path}' に保存しました。")
...
def imageflow_demo(predictor, extractor, output_video_dir, current_time, args):
    ...
    if args.save_num_emb_iou:
        plot_matching_stats(tracker.iou_count, tracker.emb_count, output_dir=args.output_dir)
