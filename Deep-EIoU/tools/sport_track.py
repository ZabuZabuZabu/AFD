...
def make_parser():
    ...
    #Detection and Embedding file
    parser.add_argument("--det_path", default=None, type=str)
...
def main():
    ...
    for seq in seqs:
        ...
        if not os.path.exists(os.path.join(data_path,args.det_path,'{}.npy'.format(seq))):
            continue

        detections = np.load(os.path.join(data_path,args.det_path,'{}.npy'.format(seq)),allow_pickle=True)
