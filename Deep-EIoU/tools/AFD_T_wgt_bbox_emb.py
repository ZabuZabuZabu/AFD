...
def make_parser():
    ...
    #追加：フォーメーション類似度の最大値と最小値の記録
    parser.add_argument(
        "--record_min_max",
        default=None,
        help="whether to record min and max value of FD",
    )
    ...
    # reid args
    parser.add_argument("--with_reid", dest="with_reid", default=True, action="store_true", help="use Re-ID flag.")
    parser.add_argument("--without_reid", dest="with_reid", action="store_false", help="disable Re-ID flag.")
...
def imageflow_demo(predictor, extractor, output_video_dir, current_time,emb_dir, args):
    ...
    while True:
        ...
        if frame_id in gt_data:
            ...
            if args.with_reid:
                ...
                # online_targets ,max_value, min_value= tracker.update(det, embs)
                online_targets = tracker.update(det, embs)

                # max_dist = max(max_dist, max_value)
                # min_dist = min(min_dist, min_value)
    ...
    if args.record_min_max:
        with open('/home/zabu/Deep-EIoU/Deep-EIoU/YOLOX_outputs/yolox_x_ch_sportsmot/for_debag/AFD/min_max_value/min_max_value.txt', 'a') as f:
            f.write(f"{video_name}: Max = {max_dist:.6f}, Min = {min_dist:.6f}\n")
