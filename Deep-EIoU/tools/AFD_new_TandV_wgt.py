...
from tracker.AFD_new import AFD_new
...
def make_parser():
    ...
    parser.add_argument("--match_thresh", type=float, default=20, help="matching threshold for tracking")
    ...
    parser.add_argument("--edge_margin", type=float, default=60, help="margin size for eliminating bbox in edge") #追加
    parser.add_argument("--magnification", type=float, default=0.1, help="magnification for GED") #追加
...
def make_center_vectors(tracks):
    """
    make formation_feature by each BBOX of tracks
    """

    if (len(tracks) > 0 and isinstance(tracks[0], np.ndarray)):
        atlbrs = tracks

    else:
        atlbrs = [track.last_tlbr for track in tracks]
    
    # Calculate center coordinates for each BBOX
    center_coords = [((bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2) for bbox in atlbrs]

    # Calculate intra-track vectors
    center_vectors = []

    for i, (x_i, y_i) in enumerate(center_coords):
        vectors = []
        for j, (x_j, y_j) in enumerate(center_coords):
            if i != j:
...
def imageflow_demo(predictor, extractor, output_video_dir, current_time, emb_dir, args):
    #データセットからgt持ってくる
    ...
    # # 保存された特徴量ファイルを読み込む
    # feature_file_path = os.path.join(emb_dir, f'{video_name}.npy')
    # if not os.path.exists(feature_file_path):
    #     raise FileNotFoundError(f"Feature file not found at {feature_file_path}")
    # all_embeddings = np.load(feature_file_path, allow_pickle=True)  # 特徴量ファイルを読み込み
    ...
    tracker = AFD_new(args, frame_rate=30)
    timer = Timer()
    frame_id = 1
    results = []
    ...
    while True:
        ...
        if frame_id in gt_data:
            ...
            try:
                # embs = all_embeddings[frame_id - 1]  # フレームIDに対応する特徴量を取得
            ...
            center_vectors = make_center_vectors(det)
            online_targets = tracker.update(det, center_vectors)
...
def main(exp, args):
    ...
    imageflow_demo(predictor, extractor, args.output_dir, current_time, emb_dir, args)
